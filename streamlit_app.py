...
import marker_upsert_utils as marker_utils  # Assuming you have this for the upload functionality

# TODO: Marvin - we can just massage this to fit the non GRAPHQL endpoint
...
def csv_to_json(csv_data):
    ...
    for index, row in csv_data.iterrows():
        ...
        event_time = datetime.now().strftime('%Y-%m-%dT%H:%M:%SZ')  # Format time in the required format
        edge = {
            "node": {
                "jsonMetadata": {
                    "description": row['INCIDENT_DESCRIPTION'],  # Incident description
...
if uploaded_file:
    ...
    with st.expander("Generated Markers JSON", expanded=False):
        ...
        st.write("Log JSON:")
        st.code(json.dumps(markers_json, indent=2), language='json')  # Log JSON to the page
    ...
    csv_data['LONGITUDE'] = pd.to_numeric(csv_data['LONGITUDE'], errors='coerce')
...
# TODO - Marvin will replace so we only use API Key and hit non-GQL endpoint
st.subheader("API Interaction")
org_uuid = st.text_input("Organization UUID:", value="ORG_UUID")
api_key = st.text_input("API Key:", type="password")
api_url = st.text_input("API URL:", value="https://your-api-endpoint.com/graphql")
...
if st.button("Upload Markers"):
    if org_uuid and api_key and api_url:
        try:
            ...
            created_count = marker_utils.upsert_markers(api_url, org_uuid, api_key, markers_list)
...
if st.button("Delete All Markers"):
    if org_uuid and api_key and api_url:
        try:
            ...
            response = marker_utils.clear_markers_and_operations(api_url, org_uuid, api_key)
            st.success(f"Successfully deleted {response['data']['clearOrgOperationsAndMarkers']['markersDeleted']} markers.")
        except Exception as e:
